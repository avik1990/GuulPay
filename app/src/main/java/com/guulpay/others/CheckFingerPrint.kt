...
class CheckFingerPrint {
    ...
    private fun generateKeyProcess() {
        keyStore = KeyStore.getInstance(Constants.Keys._KEYSTORE_TYPE)
        keyGenerator = KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, Constants.Keys._KEYSTORE_TYPE)

        keyStore.load(null)
        keyGenerator.init(KeyGenParameterSpec.Builder(cryptoPrefs.getString(Constants.Keys._KEYSTORE_ALIASNAME, ""),
    ...
    private fun generateCipher(): Cipher {
        try {
            ...
            val key = keyStore.getKey(cryptoPrefs.getString(Constants.Keys._KEYSTORE_ALIASNAME, ""), null) as SecretKey
